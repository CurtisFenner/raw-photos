import { Scanner, ScannerError, U16, U32 } from "./data.js";
import { readRealsTagExpectingSize } from "./dng.js";
...
export function splitLargeSegment(
	...
	options: { maximumArea: number }
): ImageSegment[] {
	const width = segment.x1 - segment.x0;
	const height = segment.y1 - segment.y0;
	const initialArea = width * height;
	if (initialArea <= options.maximumArea) {
		return [segment];
	}

	// Verify that the segment's encoding is not compressed.
	const [compression] = readRealsTagExpectingSize(ifd, "Compression", 1);
	if (compression !== 1) {
		// Only "Uncompressed data" is supported for segment-splitting.
		return [segment];
	}

	const [bits] = readRealsTagExpectingSize(ifd, "BitsPerSample", 1);
	const [components] = readRealsTagExpectingSize(ifd, "SamplesPerPixel", 1);
	const bitsPerPixel = bits * components;
	const bitsPerRow = bitsPerPixel * width;
	if (bitsPerRow % 8 !== 0) {
		throw new Error(
			`unsupported bitsPerRow ${bitsPerRow} = bitsPerPixel(${bitsPerPixel}) * width(${width}); must be divisible by 8`
		);
	}
	const bytesPerRow = bitsPerRow / 8;

	// TODO: This could be much larger than desirable!
	const tileHeight = 16;

	const output: ImageSegment[] = [];
	let offset = segment.offset;
	for (let y0 = segment.y0; y0 < segment.y1; y0 += tileHeight) {
		// Assume the data is stored in row-major order.
		const y1 = Math.min(y0 + tileHeight, segment.y1);
		output.push({
			x0: segment.x0,
			x1: segment.x1,
			y0,
			y1,
			offset,
			byteCount: bytesPerRow * (y1 - y0),
		});

		offset += tileHeight * bytesPerRow;
	}
...
export function readImageSegments(ifd: ImageFileDirectory): ImageSegment[] {
	...
	const imageLength = readTag(
		ifd,
		TIFF6_TAG_VALUES.ImageLength,
		readInts
	)![0];

	const rowsPerStrip = readTag(ifd, TIFF6_TAG_VALUES.RowsPerStrip, readInts);
	if (rowsPerStrip && rowsPerStrip.length === 1) {
		const stripOffsets = readTag(
			ifd,
			TIFF6_TAG_VALUES.StripOffsets,
			readInts
		);
	...
	if (
		...
		const tileOffsets = readTag(
			ifd,
			TIFF6_TAG_VALUES.TileOffsets,
			readInts
		);
